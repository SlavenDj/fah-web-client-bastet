...
export default {
	...
	methods: {
		...
		set_wide(wide) {
			this.$util.set_body_class(wide, 'theme-wide');
		},
		check_wide() {
			this.set_wide((this.$adata.config || {}).wide);
		},
		...
		check_dark() {
			...
			if (dark == undefined)
				dark = this.$util.retrieve_bool('fah-dark-mode');
			this.$util.store_bool('fah-dark-mode', dark);
			this.set_dark(dark);
		},

		toggle_dark() {
			let new_dark = !this.is_dark;
			this.set_dark(new_dark);
			this.$util.store_bool('fah-dark-mode', new_dark);

			// Keep account config in sync so it saves on the next API profile update
			if (this.$adata.config) {
				this.$adata.config.dark = new_dark;
			}
		},

		set_compact(compact) {
			this.$util.set_body_class(compact, 'theme-compact');
		},
		check_compact() {
			this.set_compact((this.$adata.config || {}).compact);
		},
		...
		open_team_chart() {
			this.$refs.team_chart_dialog.exec();
		},
				...
				switch (result.response) {
					...
					case 'register':
						...
						return this.message(
							'info',
							'Account registered',
							'<p>Your Folding@home account has ' +
					...
					default:
						return this.$account.login(result.response); // OAuth2
				}
			});
		},
		...
		async fold() {
			return this.$machs.set_state('fold');
		},
		async confirm_pause() {
			return this.$refs.pause_dialog.exec();
		},
